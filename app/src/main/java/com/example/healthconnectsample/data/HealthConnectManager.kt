...
class HealthConnectManager(private val context: Context) {
    ...
    suspend fun readHeartSeriesData(startTime: Instant, endTime: Instant): List<HeartRateRecord> {
        val timeRangeFilter = TimeRangeFilter.between(
            startTime = startTime,
            endTime = endTime
        )
    ...
    private suspend inline fun <reified T : Record> readDataWithTimeFilterOnly(
        timeRangeFilter: TimeRangeFilter,
    ): List<T> {
        val request = ReadRecordsRequest(
            recordType = T::class,
